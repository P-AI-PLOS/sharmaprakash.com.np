...
Use the named export, not the default. Don't import from the deprecated directory. Don't write `crypto.randomUUID()` — because we ship to enterprise customers on Safari builds older than my niece, and `randomUUID` is undefined in non-secure contexts, and the page just... silently breaks.
...
A hook is a shell script. It fires at a lifecycle event — `PreToolUse`, `PostToolUse`, `Stop` — and it can block, or allow, what the agent is about to do.
...
The `crypto.randomUUID` thing? Hook. Twelve lines of bash. The agent has not written `crypto.randomUUID` in our repo since the day I wrote that hook.
...
It's not the part of AI engineering that gets you a conference talk. Nobody is going to retweet your shell script that blocks `crypto.randomUUID`. Nobody is going to be impressed that you finally got the agent to stop writing barrel imports.
