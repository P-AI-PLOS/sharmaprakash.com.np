...
Eight fields. Each earns its place by changing a decision I make during the session.
...
### `cc:12k|cr:840k` — cache creation vs cache reads

Every turn Claude processes has two kinds of token cost: **cache creation** (`cc:`) tokens that are new context being written to the cache — expensive, billed at ~125% of input rate — and **cache read** (`cr:`) tokens that are reused from cache — cheap, billed at ~10% of input rate.

The ratio between them is a live health signal for your session:

- **Green `cc:`** (ratio > 20×) — the cache is working well. Most tokens are cheap reads.
- **Yellow `cc:`** (5–20×) — moderate cache busting. Something changed mid-session.
- **Red `cc:`** (< 5×) — cache is being invalidated often. Common causes: switching models, editing `CLAUDE.md` mid-session, adding/removing MCP servers. Fix the cause and the colour goes green.

A healthy session has one large `cc:` spike at the start (building initial context), then all subsequent turns show low `cc:` and high `cr:`. If you see large `cc:` on every turn, you're paying full price each time.

The field only appears when there's actual cache activity — it doesn't clutter the line at session start.
...
IFS=$'\t' read -r cwd model in_tok out_tok lines_add lines_rem ctx rl5 style cache_create cache_read < <(
  printf '%s' "$input" | jq -r '[
    .cwd // "",
    .model.display_name // "",
...
# Cache metrics: cc = creation (expensive), cr = reads (cheap)
cache_create=${cache_create:-0}; cache_read=${cache_read:-0}
cc_str=$(humanize "$cache_create")
cr_str=$(humanize "$cache_read")
...
# Cache: cc (creation=expensive) coloured by ratio; cr (reads=cheap) dimmed
if [ "$cache_create" -gt 0 ] || [ "$cache_read" -gt 0 ]; then
  if   [ "$ratio" -lt 5 ];  then cc_color=$RED
  elif [ "$ratio" -lt 20 ]; then cc_color=$YELLOW
  else                           cc_color=$GREEN
  fi
  printf " ${cc_color}cc:%s${RESET}${DIM}|cr:%s${RESET}" "$cc_str" "$cr_str"
fi
