...
import { FLAKY_TEST_ITEMS } from "~/components/tools/matrix/curated/flaky-tests.ts";
...
<AgreementCertaintyMatrix
  client:visible
  mode="curated"
  items={FLAKY_TEST_ITEMS}
  kicker="Try it"
  title="Triage shortest's flaky tests"
  instructions="Four flakiness sources off that wall. Place each one before you read the walkthrough below — then see where the room landed, and why."
/>
