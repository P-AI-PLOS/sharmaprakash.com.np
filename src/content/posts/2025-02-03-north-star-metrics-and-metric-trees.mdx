...
import MetricTreeBuilder from "~/components/tools/metric-tree/MetricTreeBuilder.tsx";
...
### Build the marketplace tree, then make it yours

The tree below opens on that marketplace decomposition — weekly transactions down through activation — so the shape is on the screen before you argue with it. Edit the nodes into your own product's metrics, add a level wherever a leaf is still too coarse for one team to own, and flag the leaves that turn out to be orphan or contested. It saves in your browser.

{/* client:load, not client:visible — full-screen mode reads its own state from a ?mtree=full query param on mount, so it needs to hydrate on page load rather than on scroll-into-view. */}
...
<MetricTreeBuilder
  ...
  source={{
    type: "post",
    postSlug: "north-star-metrics-and-metric-trees",
    postTitle: "North Star Metrics and Metric Trees",
...
2. **Draw the tree to the team level.** Decompose your North Star in [the tree above](#build-the-marketplace-tree-then-make-it-yours) until every leaf has exactly one team's name next to it. Orphan leaves — inputs nobody owns — are your unstaffed strategy; contested leaves are next quarter's coordination failure, visible early. Both are a click on the leaf.
