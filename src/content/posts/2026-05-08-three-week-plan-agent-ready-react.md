...
grep -rE "from 'components/(__v2__|__v5__|legacy|old|form)/" src --include="*.ts" --include="*.tsx" | wc -l
...
In the codebase I audited, the numbers were 3,141 / 74 / 714 / 33 / hundreds / 55. Yours will be different. The shape will be similar.
...
- How many styling systems? (SCSS files + Emotion + Tailwind + …)
...
check "Deprecated dir imports" \
  "from 'components/(__v2__|__v5__|form|legacy)/" \
  "src --include=*.ts --include=*.tsx"
...
- ❄️ **Frozen**: Redux (under `src/redux/`). Do not add to it. Do not delete it. If you touch a Redux-using file, leave the Redux usage as-is.
- 🎯 **Target**: all client state in Zustand by Q4. No active migration sprint.

## Styling
- ✅ **Current**: Emotion-in-JS via `styled` and `sx`, co-located in `style.ts`.
- ❄️ **Frozen**: SCSS partials under `src/assets/scss/`. Do not add. Do not delete.
- 🎯 **Target**: SCSS gone by Q4.

## Components
- ✅ **Current**: `atoms/`, `molecules/`, `organisms/`.
- ❄️ **Frozen**: `__v2__/`, `__v5__/`, `form/`. Do not add. Migrate to molecules as you touch them.
- 🎯 **Target**: deleted by end of Week 3 of the readiness plan.

## Forms
- ✅ **Current**: react-hook-form + Zod, fields from `molecules/Form*`.
- ❄️ **Frozen**: Formik (if applicable), inline `useState` forms.

## Tables
- ✅ **Current**: material-react-table v2 via `atoms/Table`.
...
2. **Deprecated dirs (`__v2__/`, `__v5__/`, etc.)**: for each component inside, decide one of:
   - Move to `molecules/` (or appropriate tier) with the canonical name.
   - Delete if unused (`grep -r "from 'components/__v5__/X'" src` will tell you).
...
There should be exactly one `FormTextField`, one `FormCheckbox`, one `FormSelectField` — and they all live in `molecules/`. If your form guide says "use `MuiAutocomplete`", make sure `molecules/MuiAutocomplete` is the only one and it's the canonical implementation.
