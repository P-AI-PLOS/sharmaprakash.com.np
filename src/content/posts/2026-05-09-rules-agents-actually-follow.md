...
## Stack
React 18+ · TypeScript (strict) · Vite · Chakra UI v3 ·
TanStack Query · Zustand · react-hook-form + Zod · TanStack Table
...
- No imports from `__v2__/`, `__v_old__/`, `form/`
...
For every repetitive task (migrating a list page, adding a form, scaffolding a settings page), one self-contained prompt. The agent is given this prompt verbatim plus the task scope ("Migrate the Books module").
...
The pre-commit hook is where rules become reality. A representative stack looks like:
...
> "Place permission hooks at `hooks/permissions/use<Module>Permission.ts`. Reference: `hooks/permissions/useOrderPermission.ts`."
...
> "For dropdowns, use `components/molecules/FormAutocomplete`. The atoms `Select` is legacy; do not use."
...
> "Migrate forms following the reference implementation at `pages/Reports/MonthlySummary/containers/MonthlySummaryFormPage/`."
