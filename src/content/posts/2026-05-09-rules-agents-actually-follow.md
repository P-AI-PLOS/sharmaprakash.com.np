...
For every repetitive task (migrating a list page, adding a form, scaffolding a settings page), one self-contained prompt. The agent is given this prompt verbatim plus the task scope ("Migrate the Customers module").
...
> "Place permission hooks at `hooks/permissions/use<Module>Permission.ts`. Reference: `hooks/permissions/useInvoicePermission.ts`."
...
> "Migrate forms following the reference implementation at `pages/Reports/MonthlyRollup/containers/MonthlyRollupFormPage/`."
