...
date: "2025-08-11T10:00:00+05:45"
category: ["Product"]
...
├── signals/
│   ├── 2026-W32-digest.md        # weekly digests (part three)
...
Notice what this file is doing: it's not prompt engineering, it's **onboarding**. It's the same document you'd want a contractor PM to read on day one, written tersely enough that a model keeps it in working memory. Rules 1 and 4 are your first line of defense against slop — [part six](/product-management/avoiding-ai-slop-writing-user-stories-a-team-will-actually-build/) builds the rest.

## Conventions that keep it alive

**Frontmatter on everything.** `owner`, `updated`, and tags make staleness visible (`grep` for files not touched in six months) and let commands filter — "signals tagged `billing` from the last eight weeks" is a one-liner when the metadata exists, and archaeology when it doesn't.

**Merge via pull request, even solo.** Not for ceremony — for the diff. A PR that changes `constraints.md` is a product conversation made visible, and the review habit is what keeps the repo deliberate instead of becoming a second wiki.
