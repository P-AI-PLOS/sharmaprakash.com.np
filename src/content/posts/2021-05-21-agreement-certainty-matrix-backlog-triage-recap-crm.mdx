...
import { CRM_BACKLOG_ITEMS } from "~/components/tools/matrix/curated/crm-backlog.ts";
...
<AgreementCertaintyMatrix
  client:visible
  mode="curated"
  items={CRM_BACKLOG_ITEMS}
  kicker="Try it"
  title="Sort Donut CRM's quarter backlog"
  instructions="Four real items off this backlog. Place each one before you read the walkthrough below — then see where the room actually put it, and why."
/>
