...
The first three are about the codebase. The fourth is different. A task prompt is a **runnable recipe** — a self-contained document that, when given to an agent along with a scope ("migrate the Customers list page"), produces correct code on the first attempt.
...
- `src/pages/Invoices/` — canonical reference for routing + Outlet pattern
- `src/pages/Reports/MonthlyRollup/` — most up-to-date working example
...
Every codebase has one. Maybe it's the inverted button convention. Maybe it's a custom theming pattern. Maybe it's the way pagination state lives in a Zustand store instead of in URL params, or that every page uses Jotai atoms for cross-component state instead of `useState` lifted to a parent.
...
- Can you point at a file that does it right? ("Match `pages/Reports/MonthlyRollup/containers/MonthlyRollupFormPage/`.")
...
> "Migrate the Customers list page following `docs/prompts/list-page-migration.md`."
