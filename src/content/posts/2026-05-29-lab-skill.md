...
**Recap in 30 seconds.** A command is a thin wrapper in `.claude/commands/` that routes to a skill in `.claude/skills/`. Users type `/de-barrel` and a multi-step prompt fires — loading docs, calling subagents, walking edits, running checks. The hook and subagent are infrastructure; the command is the entry point; the skill is the *handle* a teammate grabs without thinking. Deep dive: [Skills: The User-Facing Workflow Layer](/ai/skills-the-user-facing-workflow-layer/).

**Goal.** Wire `/de-barrel` by creating a thin command wrapper + a detailed skill: invoke the auditor, iterate its rows, perform each edit while the hook guards correctness, end with a green typecheck.

## Step 1 — Write the command wrapper

Create `.claude/commands/de-barrel.md` (thin entry point):
...
Delegate to the `de-barrel` skill to remove barrel imports.
```

Save the file. That's the entire command wrapper.

## Step 2 — Write the skill

Create `.claude/skills/de-barrel.md` (detailed workflow):
...
Save the file. That's the entire skill.
...
Four layers, each with one job — and refuses to do the others':

- The **command** *routes*. It's the entry point (2 lines). It delegates to the skill.
- The **skill** *orchestrates*. It knows the procedure, the order, the validation.
...
- Drop the command → the skill exists but teammates don't know about it; they have to grep the codebase to find it.
...
## What a command+skill pair can and cannot do

**Can:** orchestrate a known workflow without the user remembering its parts. Surface itself in `/help` (via the command). Be invoked by *another* skill (skills and commands compose).

**Cannot:** decide whether to *start* the workflow. That's still a human judgement — "is now a good time to de-barrel?" A command/skill pair is a verb the user wields; it isn't autonomous.

## What's next

You have the full stack now. Hook, subagent, command, skill — all four running against one rule, in one repo. The [**Capstone**](/workshop/capstone/) is a 10-minute worksheet: pick a rule *you* keep repeating in PR review, and design its hook/subagent/command+skill quadruple. No code. Just the design.
