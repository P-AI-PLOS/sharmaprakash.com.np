...
No `@mui/x-data-grid`. Reference implementation: `pages/Books/BooksList/`.
...
```tsx
<Drawer open={open} onClose={close} width="md" title="Edit book">
...
```tsx
<Dialog open={open} onClose={close} title="Delete order?" severity="error">
...
```tsx
<EmptyState
  illustration="no-books"
  title="No books yet"
  description="Add your first book to start invoicing."
  action={<Button variant="contained" color="primary">Add book</Button>}
/>
```
...
- Reference implementation: `pages/Books/BooksList/`.
```
...
```
┌──────────────────────────────────┐
│ Edit book            [×]     │  sticky header (h5, divider below)
├──────────────────────────────────┤
...
- Reference: `pages/Books/BookDrawerForm/`.
```
...
```
┌──────────────────────────────────────────────┐
│ ← Back   Book name           [⋮ actions] │  page header (h4)
├──────────────────────────────────────────────┤
│ ┌── Summary card ──┐  ┌── Stats card ──┐    │
│ │ key facts        │  │ counts / kpis  │    │
│ └──────────────────┘  └────────────────┘    │
├──────────────────────────────────────────────┤
│ Tabs: Overview | Orders | Activity         │  MUI Tabs
├──────────────────────────────────────────────┤
│ ...tab body...                               │
└──────────────────────────────────────────────┘
```
...
- Reference: `pages/Books/BookDetail/`.
```
...
- **Validation**: Zod schemas live in `features/<domain>/schemas.ts`,
  ...
  ```
- **RHF setup**: `useForm({ resolver: zodResolver(bookSchema), mode: 'all', defaultValues })`.
...
- **Submit button**: bottom of drawer/form, `contained` `primary` (accent), label is a verb (`Save`, `Create book`, never `Submit`).
