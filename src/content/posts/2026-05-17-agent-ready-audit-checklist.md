...
A typical first run on a multi-year-old codebase produces something with this shape (counts here are illustrative):

```
1. TypeScript escape hatches
  ✗  @ts-ignore / @ts-nocheck                  ~thousands
  ✗  : any / as any                            ~hundreds

2. Deprecated-dir imports
  ✗  imports from frozen directories           ~hundreds

3. Component-hierarchy duplication
  ✗  top-level dirs in src/components          double-digit
  ✗  implementations of Drawer                 3–5
  ✗  implementations of Select                 3–5
  ✗  implementations of Modal                  2–4
  !  implementations of Button                 2
  ✓  implementations of Table                  1
  !  implementations of Dialog                 2

4. Hex literals outside the theme
  ✗  hex literals in pages/components          ~hundreds

5. State libraries actively imported
  ✗  distinct state libraries in use           3–4
       redux-family                (most imports)
       atom-based store            (a handful)
       another atom store          (a handful)
       server-cache library        (broad usage)

6. Styling systems actively used
  ✗  distinct styling systems                  3
       Sass/SCSS files             (legacy bulk)
       CSS-in-JS                   (majority)
       Tailwind utilities          (small footprint)
```

That's the shape of a real first run on a codebase that has accreted over years. Don't panic. The order of operations:
