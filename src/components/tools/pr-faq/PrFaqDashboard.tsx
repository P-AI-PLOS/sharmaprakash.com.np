/**
 * "Your drafts" management grid — lists every saved PR/FAQ document,
 * lets you open or delete one. Rendered only where showDashboard is set.
 * Mirrors OstDashboard.tsx.
 */
import { FileText, Trash2 } from "lucide-react";
import { titleFor, type PrFaqRecord } from "~/utils/prfaq-store";

const formatDate = (ts: number) =>
  new Date(ts).toLocaleDateString(undefined, { month: "short", day: "numeric", year: "numeric" });

export default function PrFaqDashboard({
  records,
  activeId,
  onOpen,
  onDelete,
  ...
  return (
    <div className="mt-6 border-t border-ink-200 pt-4">
      <p className="text-caption font-semibold text-muted">Your drafts ({records.length})</p>
        ...
        {records.map((r) => (
          ...
          >
            ...
            <p className="mt-1 flex items-center gap-1 text-caption text-faint">
              ...
              {r.source.type === "post" ? "Embedded in post" : "Standalone draft"}
            </p>
