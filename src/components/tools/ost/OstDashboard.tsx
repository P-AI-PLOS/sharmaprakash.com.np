/**
 * "Your trees" management grid — lists every saved tree, links back to the
 * course chapter it was built from (if any), and lets you open or delete one.
 * Rendered only where `showDashboard` is set (the standalone tool page).
 */
import { Compass, Globe, Trash2 } from "lucide-react";
import { titleFor, type OstRecord } from "~/utils/ost-store";

const formatDate = (ts: number) =>
  new Date(ts).toLocaleDateString(undefined, { month: "short", day: "numeric", year: "numeric" });

export default function OstDashboard({
  records,
  activeId,
  onOpen,
  onDelete,
        ...
        {records.map((r) => (
          ...
          >
            ...
            <p className="mt-1 flex items-center gap-1 text-caption text-faint">
              {r.source.type === "course" ? <Compass size={12} strokeWidth={2} /> : <Globe size={12} strokeWidth={2} />}
              {r.source.type === "course" ? (
                <>
                  From{" "}
                  <a href={r.source.href} className="link-underline text-accent-700">
                    {r.source.courseTitle} · {r.source.chapterLabel}
                  </a>
                </>
              ) : (
                "Standalone tool"
