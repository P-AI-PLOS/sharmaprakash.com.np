...
// → "\n\n") and let the model handle pacing from punctuation + tags. Strip
// markdown backticks and emphasis markers — eleven_v3 reads them literally,
// which destabilizes voice (e.g. drifts into a Russian-sounding accent when a
// chunk is dense with `code` spans).
// Code spans like `crypto.randomUUID()` break eleven_v3 — the model loops or
// drifts into a foreign accent on identifier syntax. Inside backticks we drop
// empty parens and replace dots between word chars with spaces, so
// `crypto.randomUUID()` → "crypto randomUUID", `CLAUDE.md` → "CLAUDE md".
const cleanCodeSpan = (code) =>
  code
    .replace(/\(\)/g, "")
    .replace(/(\w)\.(\w)/g, "$1 $2");
const stripMarkdown = (s) =>
  s
    .replace(/`([^`]+)`/g, (_, code) => cleanCodeSpan(code))
