...
| Add or update a Marp presentation | Edit/create `src/content/presentations/<slug>.md`. Generation is explicit — **not** part of `pnpm build`. Run `pnpm presentations:build` to write `public/presentations/<slug>/index.html`, `slides.pdf`, `thumb.png`; then `pnpm presentations:check` to verify hashes match `manifest.json`. Theme: `src/styles/marp-theme.css` (mirrors `tokens.css`). Listing: `src/pages/presentations.astro`. Commit source + generated artifacts together. See `scripts/presentations/README.md`. |
