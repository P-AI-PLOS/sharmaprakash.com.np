...
- **Draw from**: Stripe Press (long-form editorial typography), Linear (warm density without
  cosplay), Vercel (confident negative space), Pirsig — *Zen and the Art of Motorcycle
  Maintenance* (the calmness behind technical writing).
- **Avoid**: Substack default (over-warm + serif everywhere), Notion marketing pages
  (cute illustrations + balloon accents), Material Design demo defaults (the curse of
  default-shaped purple cards), generic SaaS gradient heroes.
...
The other Tailwind amber steps exist in `tokens.css` and have these documented roles:

- `--accent-200` — soft amber hairline on accent-tinted surfaces: AuthorBioCard
  border, comment card border, search input border, Pagefind highlight ring,
  text-selection background. Use it when a card already has an accent wash and
  needs a border one step warmer than `--border-default`.
- `--accent-400` — currently unused. Exposed as a Tailwind color so existing
  utilities (`bg-accent-400` etc.) work if needed; no current component role.
- `--accent-800` / `--accent-900` / `--accent-950` — unused. Reserved for a
  future dark-mode pass (where accent-600 may need a darker pressed step).

Don't reach for any unused step without adding its role to this list in the
same commit.
...
| `--text-muted-on-raised` | `--ink-600`                          | secondary text on white cards (better contrast than `--text-muted`) |
...
**Last reviewed**: 2026-05-18. **Decision deadline**: 2026-12-31. If dark mode
is not wired by the deadline, delete this section — keeping it longer than that
is the kind of stale doc the audit script (Part 11 of the agent-ready-react
series) is built to flag.
...
- **Aurora drift** — two radial gradients, 14s `ease-in-out-cubic infinite alternate`,
  CSS-only.
- **Page turn** — Astro `<ClientRouter />` default + `transition:animate="fade"` on
  `<main>` (see `src/layouts/SiteShell.astro`). The fade is intentionally subtle —
  a `slide` was tried and felt too theatrical for an editorial site.

### 6.4 Banned motions

These have been proposed and rejected. Don't reintroduce without amending principle 1.

- ❌ **Parallax** on hero or anywhere else. Breaks reading on touch devices and adds
  zero information.
- ❌ **Scroll-jacking** (controlled scroll behaviour, scroll-snap on long pages).
  We never override the browser's scroll, full stop.
- ❌ **Uncapped staggered reveals on long lists.** `<ScrollReveal>` is the site's
  standard fade-up idiom (used on the homepage, About, Contact, Works, the essays
  and category indexes, series pages, etc.) — staggering is fine. What's banned is
  applying `delay={N * i}` without a cap on a list that can scroll past the
  viewport. `ScrollReveal`'s delay fires the moment IntersectionObserver triggers
  on each element, so a row 15 entries down with `delay={60 * i}` waits 900ms
  after scrolling into view before appearing — that reads as broken, not as a
  stagger. Always cap the index so only the first batch staggers and rows below
  the fold fade in immediately when they intersect: `delay={Math.min(i, 4) * 40}`
  is the canonical form. Single-screen sections (hero, NowFocus 3-card row, etc.)
  don't need the cap because every item is above the fold.
- ❌ **Animated success checkmarks** on routine actions (save, copy, submit). A
  toast or an in-place state change is enough.
- ❌ **Loading skeletons that pulse** (`animate-pulse`). Static skeletons are fine;
  pulsing skeletons are visual noise on a mostly-instant site.
- ❌ **Hover effects on touch-only contexts**. Use `@media (hover: hover)` to gate.
...
Reading-list variant used on `/blog/page/N/` and `/category/<slug>/page/N/`. Two-column
grid on `md+` (`1fr 180px`): title block on the left, 4:3 thumbnail on the right.
Mobile collapses to a single column with the thumbnail hidden (`hidden md:block`).
`h2` title, `body-lg` excerpt, category chip + date + reading-time meta row, separator
hairline (`--border-default`) between rows. Whole row is a single `<a>` so the card is
fully clickable; hover tints the title `--accent-700` and zooms the cover `scale(1.04)`.
...
- **AuthorBioCard** — 104px avatar (with a 3px surface ring + 4px `--accent-300` ring, hover bumps to `--accent-500`) + name (underlined with a 32px `--accent-500` rule) + role + bio (from `src/data/site.ts`) + LinkedIn CTA + "More essays →" ghost link. Card sits on `--surface-raised` with a soft `--accent-100` radial wash, `--accent-200` hairline, and a 4px `--accent-600` left rail.
...
SERIES POST DETAIL  (any post with `series:` + `seriesOrder:` in frontmatter)
┌───────────────────────────────────────────────────┐
│ ReadingProgressBar (sticky 2px)                    │
│ SiteHeader                                         │
│ PostHero (display-xl, byline, optional cover)     │
│ SeriesNavStrip (sunken band: "Part N of M" + chips)│ sunken
│ Body (Source Serif, max-w 68ch, TableOfContents)  │
│ ────── hairline ──────                             │
│ SeriesContinueBlock (prev/next part + full list)  │ sunken
│ SiteFooter                                        │
└───────────────────────────────────────────────────┘

RelatedPosts is intentionally suppressed on series posts to keep the foot focused
on the series itself. See `pages/[...slug].astro` for the branch.
```
...
## 9.5 Anti-patterns (banned)

The single most agent-relevant list in the file. If you find one of these in a
PR, the answer is "no" — propose a token / pattern / archetype amendment first.

- ❌ **Hardcoded hex / rgb / hsl values** in any `.astro`, `.tsx`, `.css`. Use a token.
- ❌ **Inline px or ms values** for spacing, type sizes, or durations. Use a token.
- ❌ **Accent-tinted section backgrounds**. The amber is a highlighter, never wallpaper. (See Section 1.2.)
- ❌ **Tailwind's default `shadow-*` utilities**. They're cold; the site is warm. Use `--shadow-*`. (See Section 5.)
- ❌ **CSS-in-JS, `styled-components`, UnoCSS, Emotion, CSS Modules.** The project is intentionally CSS-first with Tailwind utilities + tokenized custom CSS as the only two levers. (See Section 10.1.)
- ❌ **Marketing-page tropes**: centred-hero-with-gradient, three-feature-cards-with-icons, balloon illustrations, glowing CTA pills. If a section feels like a SaaS landing page, it's wrong. (See Section 0.)
- ❌ **Removing a `:focus-visible` ring without a replacement.** Accessibility is non-negotiable. (See Section 6.3.)
- ❌ **Decorative motion that interrupts reading.** Only the hero aurora is sanctioned. (See Section 6.1 + Section 6.4.)
- ❌ **Page archetypes not declared in Section 8.** New page shapes get an ASCII sketch in Section 8 before they're built.
- ❌ **Reaching for `--ink-200` / `--ink-300` directly** for borders or disabled states. Use `--border-default` / `--border-strong` semantic tokens. (See Section 1.3.)
...
Don't reach for CSS-in-JS, `styled-components`, Emotion, UnoCSS, or
CSS Modules — the project is intentionally CSS-first with Tailwind utilities
and tokenized custom CSS as the only two levers. (CSS Modules would be the most
tempting addition if we ever migrate to Next App Router; explicitly banned now
so the agent doesn't pre-emptively reach for them.)
