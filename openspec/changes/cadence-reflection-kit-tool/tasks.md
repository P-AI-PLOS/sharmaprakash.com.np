...
- [ ] 1.1 Verify `src/utils/pipeline-store.ts` exists (from
...
- [ ] 1.2 Check whether `okr-organizer-tool` has landed
...
- [ ] 2.3 Add pure helpers with exported types: rolling velocity (mean of last
...
- [ ] 3.1 Create `src/pages/tools/cadence-reflection-kit.astro` mirroring
...
- [ ] 3.2 Create `src/components/tools/cadence-reflection-kit/CadenceKit.tsx`:
...
- [ ] 3.3 Create `QuarterSwitcher.tsx`: list this product's cadence records in
      `compareQuarters` order; selecting a past quarter renders it read-only,
      current quarter editable.

## 4. Mode picker and metrics

- [ ] 4.1 Create `ModePicker.tsx`: one-sentence job statement + three peer
...
- [ ] 4.2 Create `MetricsPanel.tsx` — sprint variant: per-iteration
...
- [ ] 4.3 MetricsPanel — flow variant: WIP limit, observed WIP (breach
      visibly flagged), cycle-time entries per period, median cycle-time
      summary.
- [ ] 4.4 MetricsPanel — scrumban variant: flow inputs + per-period
      "planning session held?" check; no point inputs.
- [ ] 4.5 Mode switching: keep entered period data, re-render for new mode,
      show the "earlier periods were tracked under a different mode" note
      (design D3).

## 5. Retro log

- [ ] 5.1 Create `RetroLog.tsx`: entries list (went well / didn't / the one
      action + status chip), newest first.
- [ ] 5.2 New-entry form with a single action field and the one-action helper
      copy; saving stores status `open`.
- [ ] 5.3 Carry-over flow: when the latest action is `open`, surface it at
...
- [ ] 6.1 Create `DemoChecklist.tsx`: add-item form asking the key result
...
- [ ] 6.2 Empty state when the product has no OKR data: form still works,
      stores `keyResult: null`, renders the visible "not tied to a key
      result" marker + link to `/tools/okr-organizer/`.
- [ ] 6.3 D5 drift handling: re-resolve each item's ref on load; missing or
      reworded KR renders the snapshot with a "source changed/removed" badge;
      items stay editable/completable.
- [ ] 6.4 Done toggling and item delete for demo items.

## 7. Polish

- [ ] 7.1 Wrap section reveals in `<ScrollReveal>` with the
      `Math.min(i, 4) * 40` stagger cap; respect `prefers-reduced-motion`.
- [ ] 7.2 Token audit: no hardcoded colors/px; lucide-react icons only;
      component naming/namespace per AGENTS.md.
- [ ] 7.3 Page metadata: title/description in the OST page's style; confirm
      the new route doesn't collide with locked URL shapes.

## 8. Verification

- [ ] 8.1 `pnpm check` (TypeScript) passes — this is the contract-conformance
      gate (no test framework in repo, per contract risk table).
- [ ] 8.2 `pnpm build` and `pnpm preview` pass; page renders with JS disabled
      (hero + shell) and hydrates the island with JS on.
- [ ] 8.3 Manual QA script, fresh browser profile: (a) first visit seeds Donut
...
- [ ] 8.4 Verify localStorage keys used are exactly `pm-cadence-v1` and
      `pm-cadence-v1-active`, and that no other tool's keys are written.
- [ ] 8.5 Close the bd issue from 1.3 with a summary; file follow-up beads for
