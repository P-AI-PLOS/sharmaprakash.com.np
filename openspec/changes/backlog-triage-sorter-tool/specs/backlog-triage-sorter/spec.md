...
The Now/Next/Never triage board at `/tools/backlog-triage/` and embedded in
`2025-04-10-backlog-cleanup-how-to-actually-do-it.mdx`. localStorage only;
built on the `pipeline-data-contract` capability (`createToolStore`,
`ToolRecordBase`, `resolveActiveProduct`), used without modification.
