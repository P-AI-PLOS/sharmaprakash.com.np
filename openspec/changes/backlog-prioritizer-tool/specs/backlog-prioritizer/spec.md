...
The Agreement/Certainty Matrix backlog tool at `/tools/backlog-prioritizer/`:
plots Vertical Slicer stories on a Stacey 2x2, forces explicit disposition of
Complex/Chaotic items, and emits an ordered per-quarter story list, persisted
under the pipeline data contract (`pm-backlog-v1`, localStorage only).
