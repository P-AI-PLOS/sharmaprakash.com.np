...
- [ ] 1.2 Read `src/content/posts/2025-02-10-working-backwards-the-pr-faq-and-
...
- [ ] 2.1 Create `src/utils/prfaq-store.ts`: `PrFaqEntry`, `PrFaqDoc`,
      `PrFaqRecord`, `PrFaqSource` types (design D2/D3) and the store via
      `createToolStore<PrFaqRecord>({ storageKey: "pm-prfaq-v1", idPrefix:
      "prfaq" })`
- [ ] 2.2 Implement `EMPTY_DOC` seeding the five FAQ prompts as placeholder
...
- [ ] 2.3 Implement `createDoc(source)`, `saveDocData(id, doc)`,
      ...
      release"`
- [ ] 2.4 Implement FAQ entry mutators: add entry (append), remove entry by
...
- [ ] 3.1 `PrFaqBuilder.tsx` — island root modeled directly on
...
- [ ] 3.2 Implement `toMarkdown(doc: PrFaqDoc): string` per design D4 exactly
...
- [ ] 3.3 `PrFaqSwitcher.tsx` — mirrors `OstTreeSwitcher.tsx` (dropdown:
      titled records via `titleFor`, select/create/delete)
- [ ] 3.4 `PrFaqDashboard.tsx` — mirrors `OstDashboard.tsx` (management grid,
...
- [ ] 4.1 Create `src/pages/tools/pr-faq-builder.astro` mirroring
...
- [ ] 5.1 Rename `src/content/posts/2025-02-10-working-backwards-the-pr-faq-
...
- [ ] 5.3 Confirm `stripDatePrefix` in `src/content.config.ts` derives the
...
- [ ] 7.3 Cross-context scenario: create a document from the post embed,
...
- [ ] 7.4 Post-rename regression: diff the rendered post page before/after
