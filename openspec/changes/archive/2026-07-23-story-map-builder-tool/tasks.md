...
- [x] 1.2 Re-read `src/components/tools/ost/TreeBuilder.tsx`,
...
- [x] 1.3 Confirm no drag-and-drop or grid/canvas library exists in
...
- [x] 2.1 Create `src/utils/story-map-store.ts`: `BackboneStep`,
...
- [x] 2.2 (deviation, deliberate: implemented the `contextKeyFor` source union instead of the constant `"standalone"` key — the lane brief and the already-shipped `story-map.astro` `source` prop required it; design D3 text is now behind the code) Implement CRUD helpers: `createMap`, `saveMapData` (steps/slices/
...
- [x] 2.3 Implement step/slice/card mutators: add/rename/reorder step, add/
...
- [x] 2.4 Implement delete-step and delete-slice as non-cascading: removing a
      ...
      cards")
- [x] 2.5 Implement `toMarkdown(record)`: backbone as one "A → B → C" line,
...
- [x] 3.1 `StoryMapGrid.tsx` — presentational grid: CSS grid columns per
...
- [x] 3.2 `StoryMapBuilder.tsx` — editing island: title input, add/rename/
...
- [x] 3.3 `StoryMapSwitcher.tsx` (mirrors `OstTreeSwitcher.tsx`) and
      `StoryMapDashboard.tsx` (mirrors `OstDashboard.tsx`): list/create/
      switch/delete maps
- [x] 3.4 `StoryMapHelpModal.tsx` (mirrors `OstHelpModal.tsx`): explains
      backbone, slice, and the "tower of cards under one step" warning sign,
      using the post's own vocabulary
- [x] 3.5 `StoryMapExample.tsx` — hardcoded `StoryMapRecord`-shaped literal
...
- [x] 3.6 (note: `ScrollReveal` omitted — no component under `src/components/tools/` uses it; matches sibling-tool practice) Apply site conventions throughout: Tailwind utilities on
...
- [x] 4.1 Create `src/pages/tools/story-map.astro` mirroring
...
- [x] 4.3 Verify the post still renders correctly end to end (`pnpm dev`):
...
- [x] 5.1 Spec scenario walkthrough in `pnpm dev` against
...
- [x] 5.2 Non-cascading delete scenarios: delete a step with cards under it →
...
- [x] 5.3 Markdown export scenario: build the exact onboarding vignette from
      the post, export, and confirm backbone line + slice section order +
      backlog section match design D7's example output
- [x] 5.4 Post-embed scenarios: confirm `StoryMapExample` renders read-only
      ...
      shows on reload
- [x] 5.5 (note: `pnpm check` shows pre-existing errors in `spec-builder/*`, `spec-store.ts`, `content.config.ts` — none in story-map files; `pnpm build` passes) Quality gates: `pnpm check` and `pnpm build` pass (repo has no test
