...
A Patton-style story map tool at `/tools/story-map/`: a backbone of user
activities (columns), story cards under each activity, and one or more named
release slices that partition cards into releases, plus an unsliced backlog
band. localStorage only; built on the `pipeline-data-contract` capability
(`createToolStore`, `ToolRecordBase`, `resolveActiveProduct`, `uid`). Also
covers the read-only worked-example embed used in the story mapping post.
