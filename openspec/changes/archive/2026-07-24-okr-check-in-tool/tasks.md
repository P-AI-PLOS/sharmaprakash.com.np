...
- [x] 1.1 Confirm `donut-crm-pipeline-data-contract` is applied:
...
- [x] 1.2 Confirm `okr-organizer-tool` is applied: `src/utils/okr-store.ts`
...
- [x] 2.1 Create `src/utils/checkin-store.ts`: `CheckInRecord`,
...
- [x] 2.2 Implement `resolveCheckIn(productId, okr)` — returns the existing
      ...
      objective snapshot
- [x] 2.3 Implement entry mutators: set actual / confidence / reflection,
...
- [x] 2.4 Implement draft handoff `draftNextQuarterOkr(checkin, includedKrIds)`
...
- [x] 2.5 Implement `resolveEntryRef(entry)` — live text / "changed" /
...
- [x] 3.1 `CheckInTool.tsx` — island root: `resolveActiveProduct()`, OKR
      ...
      via `quarterKey`
- [x] 3.2 `KeyResultCheckInCard.tsx` — resolved/badged KR text, snapshot log
...
- [x] 3.3 `NextQuarterDraftPanel.tsx` — per-entry include checkboxes
      ...
      `/tools/okr-organizer/`
- [x] 3.4 `CheckInSwitcher.tsx` — prior check-ins for the product sorted by
...
- [x] 3.5 Apply site conventions: Tailwind utilities on `tokens.css`
      variables only, `<ScrollReveal delay={Math.min(i, 4) * 40}>` on entry
      lists, `prefers-reduced-motion` respected

## 4. Page

- [x] 4.1 Create `src/pages/tools/okr-check-in.astro` mirroring
...
- [x] 5.2 Draft handoff scenarios: draft from a Q3 check-in → Q4 record with
      ...
      untouched
- [x] 5.3 Stale-ref scenarios: delete a KR and the whole OKR in the organizer
      → badges shown, logged data intact; add a KR → sync appends without
      touching existing entries
- [x] 5.4 Quality gates: `pnpm check` and `pnpm build` pass (repo has no test
...
- [x] 5.5 Close-out: no beads existed for this work at proposal time — file
