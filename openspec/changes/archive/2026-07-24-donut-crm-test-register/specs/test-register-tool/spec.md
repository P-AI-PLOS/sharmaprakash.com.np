...
The QA-ring Test Register: a browser-local, per-product register of test
scenarios tied to Vertical Slicer stories and Spec Builder acceptance
criteria, with snapshot-based staleness detection and a coverage gap rollup.
Served at `/tools/test-register/`; persists under `pm-testreg-v1` per the
pipeline data contract. Planning and tracking only — the tool SHALL NOT
execute tests.
