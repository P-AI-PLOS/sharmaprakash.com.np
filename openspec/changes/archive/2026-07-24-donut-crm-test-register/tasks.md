...
- [x] 1.1 Create `src/utils/test-register-store.ts`: `AutomationStatus`,
...
- [x] 1.2 Create `src/components/tools/test-register/sources.ts` (or
...
- [x] 1.3 Implement `resolveScenarioLinks(rec)` as a pure function returning
...
- [x] 1.4 Implement the lifecycle mutations on top of the factory:
...
- [x] 2.1 `StatusBadge.tsx` — single source of truth for the four displayed
...
- [x] 2.2 `ScenarioEditor.tsx` — description, story picker and criterion
...
- [x] 2.3 `ScenarioList.tsx` — the register: description, linked
...
- [x] 2.4 Stale-resolution UI in the list/editor: side-by-side snapshot vs.
...
- [x] 2.5 `CoveragePanel.tsx` — per-spec gap lists (uncovered first, then
...
- [x] 2.6 `RegisterHelpModal.tsx` — operational quick reference (statuses,
      actions, "refresh after editing specs" note), mirroring
      `OstHelpModal.tsx` structure
- [x] 2.7 `TestRegister.tsx` — root island: `resolveActiveProduct()`, load
      scenarios via `listForProduct`, register/coverage tab state, wire all
      children; `client:load`

## 3. Page

- [x] 3.1 Create `src/pages/tools/test-register.astro` on the
...
- [x] 3.2 Write the teaching content section (design.md D6): running a test
...
- [x] 3.3 Cross-link: teaching content references the Spec Builder and
...
- [x] 4.1 Emit via `trackEvent` (`src/utils/analytics.ts`):
...
- [x] 5.1 `pnpm check` passes (contract types consumed with no `any` leaks;
      `resolveScenarioLinks` fully typed)
- [x] 5.2 `pnpm build` passes; confirm `/tools/test-register/index.html` is
      emitted and contains the teaching content statically
- [x] 5.3 Manual smoke script in `pnpm dev` (no test framework in repo —
...
- [x] 5.4 Verify `resolveScenarioLinks` link-state matrix in the browser
      console against hand-built fixtures (live/changed/removed × story/
      criterion), since it is the tool's core logic
- [x] 5.5 Confirm no route/URL changes elsewhere, no edits to
      `ost-store.ts`, `pipeline-store.ts`, or sibling tool files outside the
      `sources.ts` adapter's imports
- [x] 5.6 Close-out: no open beads exist for this work today; filed follow-up
      beads for (a) cross-tool Donut CRM sample scenarios (blocked on
      sibling sample-data decisions) → `blog-9jg`, (b) `pipeline-tools-chrome`
      participation for this page → `blog-xiy`, (c) the stale-fraction display
      question from design.md Open Questions → `blog-wsw` — each citing this
      change and the archived data contract as records
