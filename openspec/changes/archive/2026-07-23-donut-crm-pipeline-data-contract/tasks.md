...
- [x] 1.1 Create `src/utils/pipeline-store.ts` with the module doc comment
...
- [x] 3.1 Implement `quarterKey` ("YYYY-QN"), `parseQuarterKey` (null on
...
- [x] 4.1 Implement `createToolStore<T extends ToolRecordBase>({ storageKey,
...
- [x] 4.2 Implement the per-product active pointer on `<storageKey>-active`
      (a `Record<productId, recordId>` map): `getActiveId(productId)`,
      `setActiveId(productId, id)`

## 5. Migrate `ost-store.ts` onto the factory (D9)

- [x] 5.1 Before touching code: export/snapshot a real browser's
      `ost-trees-v1`/`ost-active-v1` localStorage as a fixture for manual
      before/after comparison
- [x] 5.2 Add stable ids to tree nodes: `OstOpportunity` and each solution
...
- [x] 5.3 Add `productId` to `OstRecord` (satisfying `ToolRecordBase`); backfill
      existing records to `resolveActiveProduct().id` in the same pass
- [x] 5.4 Re-implement `ost-store.ts`'s CRUD (`listTrees`, `getTree`,
...
- [x] 5.5 Re-implement `getActiveId`/`setActiveId`/`resolveActiveTree` using
...
- [x] 5.6 Update `OstPickRef` (D2) from index-addressed to
...
- [x] 5.7 Manual verification against the 5.1 fixture: load the pre-migration
...
- [x] 6.1 Type-check the contract against real consumers: `pnpm check` passes,
...
- [x] 6.2 Run `pnpm build` to confirm the static build is unaffected
- [x] 6.3 Manual smoke in `pnpm dev` browser console: import path resolves,
...
- [x] 6.4 Close-out: no open beads exist for this work today; file follow-up
