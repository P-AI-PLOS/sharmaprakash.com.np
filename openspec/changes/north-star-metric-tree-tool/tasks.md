...
- [ ] 1.1 Confirm `pipeline-data-contract` is applied and unchanged:
...
- [ ] 1.2 Confirm `react-d3-tree` is already a dependency (via OST) — no
      `pnpm add` needed for this change

## 2. Store (`src/utils/metric-tree-store.ts`)

- [ ] 2.1 Define types per design D2/D3: `MetricAnnotationStatus`,
...
- [ ] 2.3 Implement node helpers: `newNode(text)` (uid("mtn")), `addChild(tree, parentId, text)`,
...
- [ ] 2.4 Implement the annotation-clear-on-decompose rule (design D5):
      `addChild` clears `annotation` on the parent it attaches to
- [ ] 2.5 Implement `resolveActiveTree(source)` mirroring `ost-store.ts`'s
...
- [ ] 2.6 Implement `toMarkdown(tree)` per design D6 (root/branch/leaf icons,
      depth-indented bullets, orphan/contested inline markers)
- [ ] 2.7 Implement `listTrees`, `getTree`, `createTree`, `saveTreeData`,
...
- [ ] 3.1 `MetricTreeDiagram.tsx` — adapt `TreeDiagram.tsx`'s react-d3-tree
...
- [ ] 3.2 `MetricTreeBuilder.tsx` — the island root, adapted from
...
- [ ] 3.3 `MetricTreeHelpModal.tsx` — mirrors `OstHelpModal.tsx`: concepts for
...
- [ ] 3.4 `MetricTreeDashboard.tsx` — mirrors `OstDashboard.tsx`: "your
      trees" grid for the standalone page, showing source (standalone vs.
      which post), updated date, open/delete actions
- [ ] 3.5 `MetricTreeSwitcher.tsx` — mirrors `OstTreeSwitcher.tsx`: create /
...
- [ ] 4.1 Create `src/pages/tools/north-star-metric-tree.astro` mirroring
...
- [ ] 5.1 Rename `src/content/posts/2025-02-03-north-star-metrics-and-metric-trees.md`
      ...
      unchanged
- [ ] 5.2 Add `import MetricTreeBuilder from "~/components/tools/metric-tree/MetricTreeBuilder.tsx";`
...
- [ ] 5.3 Lightly adjust the surrounding prose so the "Draw the tree to the
...
- [ ] 6.2 Post-embed seeding scenarios: clear localStorage, load the post,
...
- [ ] 6.3 Export scenario: copy Markdown from a tree with one orphan and one
      contested leaf; confirm indentation-by-depth and distinct markers
- [ ] 6.4 Quality gates: `pnpm check` and `pnpm build` pass (no test
...
- [ ] 6.5 Close-out: file beads for follow-ups surfaced during
